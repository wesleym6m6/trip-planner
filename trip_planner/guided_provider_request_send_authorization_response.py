"""Exact response to one fresh private provider-request send review.

Phase 5.24 captures only an unambiguous accept-send, request-smaller, or cancel
choice for the exact current Phase 5.23 review.  Capture and assessment
revalidate the complete private chain, the same typed target preimages, the
materialized non-sendable contracts, and review freshness.

Accept-send permits only preparation of a separate provider-request send gate.
It does not make a contract sendable, choose transport or an HTTP method, bind
or read a credential, create an HTTP request, call a provider, reserve spend,
persist data, schedule, render, deploy, confirm, or mutate canonical state.
Request-smaller changes no target or cap by itself; cancel closes only the
current send path.
"""
...
_ACCEPT_ACTION = "prepare_private_provider_request_send_preparation"
...
class GuidedProviderRequestSendAuthorizationResponseKind(str, Enum):
    """One exact choice from the visible Phase 5.23 private review."""
...
class GuidedProviderRequestSendAuthorizationResponseStatus(str, Enum):
    """Non-executable handoff after one exact send-authorization response."""
    ...
    PROVIDER_REQUEST_SEND_CANCELLED = "provider_request_send_cancelled"


@dataclass(frozen=True, slots=True, repr=False)
class GuidedProviderRequestSendAuthorizationResponse:
    ...
    def __post_init__(self, _token: object | None) -> None:
        if _token is not _RESPONSE_TOKEN:
            raise ValueError("Send-authorization responses require trusted capture")
        if type(self.kind) is not GuidedProviderRequestSendAuthorizationResponseKind:
            raise TypeError("Send-authorization response kind must be exact")
...
@dataclass(frozen=True, slots=True, repr=False)
class GuidedProviderRequestSendAuthorizationResponseReview:
    """Aggregate safe handoff with no active send or execution authority."""

    status: GuidedProviderRequestSendAuthorizationResponseStatus
    response_kind: GuidedProviderRequestSendAuthorizationResponseKind
    next_action: str
    ...
    def __post_init__(self, _token: object | None) -> None:
        if _token is not _REVIEW_TOKEN:
            raise ValueError("Send-authorization response reviews require the assessor")
        if (
            type(self.status) is not GuidedProviderRequestSendAuthorizationResponseStatus
            or type(self.response_kind)
            is not GuidedProviderRequestSendAuthorizationResponseKind
        ):
            raise TypeError("Send-authorization response enums must be exact")
        for name, value, maximum in (
            ...
            if type(value) is not int or not 1 <= value <= maximum:
                raise ValueError(f"Send-authorization response {name} is invalid")
        for name, value in (
        ...
        for name, value in (
            ...
            if type(value) is not int or not 0 <= value <= _MAX_SOURCE_LINE_REFERENCES:
                raise ValueError(f"Send-authorization response {name} is invalid")
        if (
            self.accepted_scope_item_count > self.contract_count
            ...
            raise ValueError("Send-authorization response counts differ")
        ...
        if (
            capability_counts[GuidedProviderCapability.SERPAPI_GOOGLE_HOTELS]
            != self.bound_serpapi_request_count
            or sum(
            ...
            raise ValueError("Send-authorization response provider counts differ")
        if (
            ...
            raise ValueError("Send-authorization response categories are invalid")
        if (
            ...
            raise ValueError("Send-authorization response cost/freshness is invalid")
        expected = {
        ...
        if (self.status, self.next_action) != expected:
            raise ValueError("Send-authorization response status/action conflict")
        if (
            ...
            raise ValueError("Send-authorization response safe metadata is invalid")
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        accepted = (
            self.response_kind
            is GuidedProviderRequestSendAuthorizationResponseKind
            .ACCEPT_SEND
        )
        smaller = (
            self.response_kind
            is GuidedProviderRequestSendAuthorizationResponseKind.REQUEST_SMALLER
        )
        cancelled = (
            self.response_kind
            is GuidedProviderRequestSendAuthorizationResponseKind.CANCEL
        )
        return {
            ...
            "provider_request_send_authorization_response": {
                ...
                "send_authorization_response_captured": True,
                "accepted_exact_private_send_authorization_review": accepted,
                "accepted_for_separate_send_preparation": accepted,
                "requested_smaller_execution_target_set": smaller,
                "current_request_send_cancelled": cancelled,
                "may_prepare_private_provider_request_send_preparation": accepted,
                "may_refine_private_provider_execution_targets": smaller,
                "current_request_send_path_closed": cancelled,
                ...
                "authorization_chain_unchanged_by_response": True,
                "materialized_request_contracts_unchanged_by_response": True,
                "request_caps_unchanged_by_response": True,
                "transport_selection_unchanged_by_response": True,
                "credential_state_unchanged_by_response": True,
...
def capture_guided_provider_request_send_authorization_response(
    ...
    *,
    ...
    if type(kind) is not GuidedProviderRequestSendAuthorizationResponseKind:
        raise TypeError("kind must be an exact send-authorization response kind")
    captured_at = _utc_datetime(evaluation_at, "evaluation_at")
    assessed_review = assess_guided_provider_request_send_authorization_review(
        brief,
        cards,
        preference,
        refinement,
    ...
    if (
        ...
        raise ValueError("Send-authorization response requires the current review")
    return GuidedProviderRequestSendAuthorizationResponse(
        kind=kind,
...
def assess_guided_provider_request_send_authorization_response(
    brief: TripBriefDraft,
    cards: tuple[GuidedDirectionCard, ...],
    preference: GuidedDirectionPreference,
    refinement: GuidedRefinementCandidate,
    ...
    materialization: GuidedProviderRequestContractMaterialization,
    send_authorization_review: GuidedProviderRequestSendAuthorizationReview,
    response: GuidedProviderRequestSendAuthorizationResponse,
    *,
    preimages: tuple[GuidedProviderExecutionTargetPreimage, ...],
    evaluation_at: datetime,
) -> GuidedProviderRequestSendAuthorizationResponseReview:
    ...
    if type(response) is not GuidedProviderRequestSendAuthorizationResponse:
        raise TypeError("response must be an exact send-authorization response")
    ...
    captured_review = assess_guided_provider_request_send_authorization_review(
        brief,
        cards,
        preference,
        refinement,
    ...
    if (
        ...
        raise ValueError("Send-authorization response requires its original review")
    ...
    if response._context_fingerprint != expected_fingerprint:
        raise ValueError("Send-authorization response differs from exact context")

    current_review = assess_guided_provider_request_send_authorization_review(
        brief,
        cards,
        preference,
        refinement,
    ...
    source = current_review._materialization_review
    return GuidedProviderRequestSendAuthorizationResponseReview(
        status=status,
        ...
        user_stated_source_line_count=source.user_stated_source_line_count,
        tentative_source_line_count=source.tentative_source_line_count,
        ai_candidate_source_line_count=source.ai_candidate_source_line_count,
        estimated_bound_first_paid_tier_google_cost_usd_micros=(
        ...
        serpapi_bound_plan_credit_count=source.serpapi_bound_plan_credit_count,
        serpapi_plan_credit_cap=source.serpapi_plan_credit_cap,
        bound_request_capability_counts=source.bound_request_capability_counts,
    ...
    return _sha256(
        {
            ...
            "send_authorization_review_contract_version": (
                GUIDED_PROVIDER_REQUEST_SEND_AUTHORIZATION_REVIEW_VERSION
            ),
            "domain": "guided-provider-request-send-authorization-response",
...
def _validate_capability_counts(
    ...
    if (
        ...
        raise ValueError("Send-authorization response capability counts are invalid")
