"""Exact response to one fresh private request-materialization review.

Phase 5.21 captures only an unambiguous prepare-materialization,
request-smaller, or cancel choice for the exact current Phase 5.20 review.
Capture and assessment revalidate the complete private chain, the same typed
target preimages, the execution-time recheck, and review freshness.

Prepare-materialization permits only preparation of a separate exact provider
request-contract materialization gate.  It does not create an executable
provider contract or HTTP request, select transport, read a credential, call a
provider, reserve spend, persist data, schedule, render, deploy, confirm, or
mutate canonical state.  Request-smaller changes no target or cap by itself;
cancel closes only the current materialization path.
"""
...
_PREPARE_ACTION = "prepare_private_provider_request_contract_materialization"
...
class GuidedProviderRequestMaterializationResponseKind(str, Enum):
    """One exact choice from the visible Phase 5.20 private review."""

    PREPARE_MATERIALIZATION = "prepare_materialization"
    REQUEST_SMALLER = "request_smaller"
    CANCEL = "cancel"


class GuidedProviderRequestMaterializationResponseStatus(str, Enum):
    """Non-executable handoff after one exact materialization response."""
...
class GuidedProviderRequestMaterializationResponse:
    ...
    def __post_init__(self, _token: object | None) -> None:
        if _token is not _RESPONSE_TOKEN:
            raise ValueError("Materialization responses require trusted capture")
        if type(self.kind) is not GuidedProviderRequestMaterializationResponseKind:
            raise TypeError("Materialization response kind must be exact")
...
@dataclass(frozen=True, slots=True, repr=False)
class GuidedProviderRequestMaterializationResponseReview:
    """Aggregate safe handoff with no request or execution authority."""

    status: GuidedProviderRequestMaterializationResponseStatus
    response_kind: GuidedProviderRequestMaterializationResponseKind
    next_action: str
    ...
    def __post_init__(self, _token: object | None) -> None:
        if _token is not _REVIEW_TOKEN:
            raise ValueError("Materialization response reviews require the assessor")
        if (
            type(self.status) is not GuidedProviderRequestMaterializationResponseStatus
            or type(self.response_kind)
            is not GuidedProviderRequestMaterializationResponseKind
        ):
            raise TypeError("Materialization response enums must be exact")
        for name, value, maximum in (
            ...
            if type(value) is not int or not 1 <= value <= maximum:
                raise ValueError(f"Materialization response {name} is invalid")
        for name, value in (
        ...
        if (
            ...
            raise ValueError("Materialization response counts differ")
        ...
        if (
            ...
            raise ValueError("Materialization response provider counts differ")
        if (
            ...
            raise ValueError("Materialization response categories are invalid")
        if (
            ...
            raise ValueError("Materialization response cost/freshness is invalid")
        expected = {
        ...
        if (self.status, self.next_action) != expected:
            raise ValueError("Materialization response status/action conflict")
        if (
            ...
            raise ValueError("Materialization response safe metadata is invalid")
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        prepare = (
            self.response_kind
            is GuidedProviderRequestMaterializationResponseKind
            .PREPARE_MATERIALIZATION
        )
        smaller = (
            self.response_kind
            is GuidedProviderRequestMaterializationResponseKind.REQUEST_SMALLER
        )
        cancelled = (
            self.response_kind
            is GuidedProviderRequestMaterializationResponseKind.CANCEL
        )
        return {
            ...
            "provider_request_materialization_response": {
                ...
                "eligible_for_request_contract_materialization_count": (
                    self.bound_request_count if prepare else 0
                ),
                ...
                "request_contract_candidates_unchanged_by_response": True,
                ...
                "immediate_request_materialization_authority_granted": False,
                "execution_authority_active": False,
                "partial_materialization_authorization_permitted": False,
                "private_target_values_exposed": False,
                "provider_identifier_values_exposed": False,
                "target_fingerprints_exposed": False,
                "request_contract_candidate_count_created_by_response": 0,
            ...
            "side_effects": {
                ...
                "request_contract_candidates_created": False,
...
def capture_guided_provider_request_materialization_response(
    ...
    *,
    ...
    if type(kind) is not GuidedProviderRequestMaterializationResponseKind:
        raise TypeError("kind must be an exact materialization response kind")
    captured_at = _utc_datetime(evaluation_at, "evaluation_at")
    assessed_review = assess_guided_provider_request_materialization_review(
        brief,
        cards,
        preference,
        refinement,
    ...
    if (
        ...
        raise ValueError("Materialization response requires the current review")
    return GuidedProviderRequestMaterializationResponse(
        kind=kind,
...
def assess_guided_provider_request_materialization_response(
    brief: TripBriefDraft,
    cards: tuple[GuidedDirectionCard, ...],
    preference: GuidedDirectionPreference,
    refinement: GuidedRefinementCandidate,
    ...
    execution_time_recheck: GuidedProviderExecutionTimeRecheck,
    materialization_review: GuidedProviderRequestMaterializationReview,
    response: GuidedProviderRequestMaterializationResponse,
    *,
    preimages: tuple[GuidedProviderExecutionTargetPreimage, ...],
    evaluation_at: datetime,
) -> GuidedProviderRequestMaterializationResponseReview:
    ...
    if type(response) is not GuidedProviderRequestMaterializationResponse:
        raise TypeError("response must be an exact materialization response")
    ...
    captured_review = assess_guided_provider_request_materialization_review(
        brief,
        cards,
        preference,
        refinement,
    ...
    if (
        ...
        raise ValueError("Materialization response requires its original review")
    ...
    if response._context_fingerprint != expected_fingerprint:
        raise ValueError("Materialization response differs from exact context")

    current_review = assess_guided_provider_request_materialization_review(
        brief,
        cards,
        preference,
        refinement,
    ...
    if (
        ...
        raise ValueError("Materialization review is no longer current")
    status, next_action = {
        GuidedProviderRequestMaterializationResponseKind.PREPARE_MATERIALIZATION: (
            GuidedProviderRequestMaterializationResponseStatus
            .READY_FOR_PRIVATE_PROVIDER_REQUEST_CONTRACT_MATERIALIZATION,
    ...
    return GuidedProviderRequestMaterializationResponseReview(
        status=status,
        ...
        candidate_count=current_review.candidate_count,
        ...
        user_stated_source_line_count=(
            current_review.user_stated_source_line_count
        ),
        tentative_source_line_count=current_review.tentative_source_line_count,
        ai_candidate_source_line_count=(
            current_review.ai_candidate_source_line_count
        ),
        estimated_bound_first_paid_tier_google_cost_usd_micros=(
            current_review.estimated_bound_first_paid_tier_google_cost_usd_micros
        ),
        accepted_max_first_paid_tier_google_cost_usd_micros=(
            current_review.accepted_max_first_paid_tier_google_cost_usd_micros
        ),
        serpapi_bound_plan_credit_count=(
        ...
        bound_request_capability_counts=(
            current_review.bound_request_capability_counts
        ),
    ...
    return _sha256(
        {
            ...
            "materialization_review_contract_version": (
                GUIDED_PROVIDER_REQUEST_MATERIALIZATION_REVIEW_VERSION
            ),
            "domain": "guided-provider-request-materialization-response",
...
def _validate_capability_counts(
    ...
    if (
        ...
        raise ValueError("Materialization response capability counts are invalid")
