"""Capture one exact response to a fresh credential-binding review.

Phase 5.27 accepts only an unambiguous typed accept-credential-binding,
request-smaller, or cancel choice for the exact current Phase 5.26 review.
Capture and assessment revalidate the complete private chain, the same typed
target preimages, the same transport-bound contracts, and inherited freshness.

Acceptance permits only preparation of a separate explicit live credential-
binding gate.  It does not read or bind a credential value, expand provider
identifiers, construct an HTTP request, open a network connection, call a
provider, reserve spend, persist data, schedule, render, deploy, confirm, or
mutate canonical state.  Reduction and cancellation also grant no authority.
"""
...
_MAX_REQUESTS = 32
_ACCEPT_ACTION = "prepare_private_provider_request_live_credential_binding_gate"
...
class GuidedProviderRequestCredentialBindingResponseKind(str, Enum):
    """One exact choice from the visible Phase 5.26 private review."""

    ACCEPT_CREDENTIAL_BINDING = "accept_credential_binding"
    REQUEST_SMALLER = "request_smaller"
    CANCEL = "cancel"


class GuidedProviderRequestCredentialBindingResponseStatus(str, Enum):
    """Non-executable handoff after one exact credential-binding response."""
...
class GuidedProviderRequestCredentialBindingResponse:
    ...
    def __post_init__(self, _token: object | None) -> None:
        if _token is not _RESPONSE_TOKEN:
            raise ValueError(
                "Credential-binding responses require trusted capture"
            )
        if type(self.kind) is not GuidedProviderRequestCredentialBindingResponseKind:
            raise TypeError("Credential-binding response kind must be exact")
...
@dataclass(frozen=True, slots=True, repr=False)
class GuidedProviderRequestCredentialBindingResponseReview:
    """Aggregate safe handoff with no credential or send authority."""

    status: GuidedProviderRequestCredentialBindingResponseStatus
    response_kind: GuidedProviderRequestCredentialBindingResponseKind
    next_action: str
    ...
    def __post_init__(self, _token: object | None) -> None:
        if _token is not _REVIEW_TOKEN:
            raise ValueError(
                "Credential-binding response reviews require the assessor"
            )
        if (
            type(self.status)
            ...
            raise TypeError("Credential-binding response enums must be exact")
        ...
        if (
            ...
            raise ValueError(
                "Credential-binding response requires an accepted send review"
            )
        ...
        if (
            ...
            raise ValueError("Credential-binding response version differs")
    ...
    def to_dict(self) -> dict[str, Any]:
        """Return safe aggregate branch semantics without private values."""

        source = self._send_authorization_response_review
        accepted = (
            self.response_kind
            is GuidedProviderRequestCredentialBindingResponseKind
            .ACCEPT_CREDENTIAL_BINDING
        )
        smaller = (
            self.response_kind
            is GuidedProviderRequestCredentialBindingResponseKind.REQUEST_SMALLER
        )
        cancelled = (
            self.response_kind
            is GuidedProviderRequestCredentialBindingResponseKind.CANCEL
        )
        return {
            ...
            "provider_request_credential_binding_response": {
                ...
                "credential_binding_response_captured": True,
                "accepted_exact_private_credential_binding_review": accepted,
                "accepted_for_separate_live_credential_binding_gate": accepted,
                ...
                "credential_binding_path_cancelled": cancelled,
                ...
                "private_transport_metadata_retained_by_response": False,
            ...
            "side_effects": {
                "process_local": True,
                "credential_binding_review_read": True,
...
def capture_guided_provider_request_credential_binding_response(
    ...
    *,
    ...
    if type(kind) is not GuidedProviderRequestCredentialBindingResponseKind:
        raise TypeError(
            "kind must be an exact credential-binding response kind"
        )
    captured_at = _utc_datetime(evaluation_at, "evaluation_at")
    assessed_review = assess_guided_provider_request_credential_binding_review(
        brief,
        cards,
        preference,
        refinement,
    ...
    if (
        ...
        is not GuidedProviderRequestCredentialBindingReviewStatus.REVIEW_REQUIRED
    ):
        raise ValueError(
            "Credential-binding response requires the current review"
        )
    return GuidedProviderRequestCredentialBindingResponse(
        kind=kind,
...
def assess_guided_provider_request_credential_binding_response(
    brief: TripBriefDraft,
    cards: tuple[GuidedDirectionCard, ...],
    preference: GuidedDirectionPreference,
    refinement: GuidedRefinementCandidate,
    ...
    send_preparation: GuidedProviderRequestSendPreparation,
    credential_binding_review: GuidedProviderRequestCredentialBindingReview,
    response: GuidedProviderRequestCredentialBindingResponse,
    *,
    preimages: tuple[GuidedProviderExecutionTargetPreimage, ...],
    evaluation_at: datetime,
) -> GuidedProviderRequestCredentialBindingResponseReview:
    ...
    if type(response) is not GuidedProviderRequestCredentialBindingResponse:
        raise TypeError("response must be an exact credential-binding response")
    ...
    captured_review = assess_guided_provider_request_credential_binding_review(
        brief,
        cards,
        preference,
        refinement,
    ...
    if (
        ...
        raise ValueError(
            "Credential-binding response requires its original review"
        )
    ...
    if response._context_fingerprint != expected_fingerprint:
        raise ValueError(
            "Credential-binding response differs from exact context"
        )

    current_review = assess_guided_provider_request_credential_binding_review(
        brief,
        cards,
        preference,
        refinement,
    ...
    source = (
        current_review._preparation_review
        ._send_authorization_response_review
    )
    return GuidedProviderRequestCredentialBindingResponseReview(
        status=status,
        response_kind=response.kind,
        next_action=next_action,
        transport_profile_counts=(
            current_review._preparation_review.transport_profile_counts
        ),
        _send_authorization_response_review=source,
...
def _response_branch(
    kind: GuidedProviderRequestCredentialBindingResponseKind,
) -> tuple[GuidedProviderRequestCredentialBindingResponseStatus, str]:
    return {
    ...
    return _sha256(
        {
            ...
            "credential_binding_review_contract_version": (
                GUIDED_PROVIDER_REQUEST_CREDENTIAL_BINDING_REVIEW_VERSION
            ),
            "domain": "guided-provider-request-credential-binding-response",
...
def _validate_profile_counts(
    ...
    if (
        ...
        raise ValueError(
            "Credential-binding response transport profile counts are invalid"
        )
